...
if "!_NCS!"=="0" (
    set "cGreen=" & set "cRed=" & set "cYellow=" & set "cCyan=" & set "cMagenta=" & set "cWhite=" & set "cOrange=" & set "cGreyHL=" & set "cReset="
)
...
:show_title
cls
%psc% "&{$W=$Host.UI.RawUI.WindowSize;$B=$Host.UI.RawUI.BufferSize;$W.Height=55;$W.Width=100;$B.Height=300;$B.Width=100;$Host.UI.RawUI.WindowSize=$W;$Host.UI.RawUI.BufferSize=$B;}" >nul 2>&1
...
echo.
echo  !cWhite! 2.!cReset! Al desactivar las caracteristicas se te pedira reiniciar.
echo     En el siguiente arranque presiona !cYellow!F7!cReset! para deshabilitar
echo     la firma obligatoria de controladores.
echo.
echo  !cWhite! 3.!cReset! Para revertir todos los cambios usa la opcion !cCyan![4]!cReset!.
echo     Tambien puedes usar !cCyan![2] Activar Todo!cReset! para restaurar
echo     las protecciones manualmente.
echo.
echo  !cWhite! 4.!cReset! !cRed!IMPORTANTE - INTERNET:!cReset!
echo     !cRed! Desconecta el cable de red o desactiva el Wi-Fi!cReset!
echo     !cRed! antes de usar este script. Vuelve a conectarte!cReset!
echo     !cRed! una vez hayas reactivado todas las protecciones.!cReset!
echo  ----------------------------------------------------------------
exit /b

:: ========================================
:: VERIFICACION VT-x / SVM EN BIOS
:: ========================================

:check_vtx
set "vtx=0"
set "hvpresent=0"
for /f "delims=" %%s in ('%psc% "(gcim Win32_ComputerSystem).HypervisorPresent" 2^>nul') do (
    if /i "%%s"=="True" set "hvpresent=1"
)
for /f "delims=" %%s in ('%psc% "(Get-CimInstance -ClassName Win32_Processor).VirtualizationFirmwareEnabled" 2^>nul') do (
...
:: ========================================
:: VERIFICACION Y SUSPENSION DE BITLOCKER
:: Parametro 1: cantidad de reinicios (1 o 2)
:: ========================================
...
:suspend_bitlocker
set "bl_rebootcount=%~1"
if "!bl_rebootcount!"=="" set "bl_rebootcount=1"
call :check_bitlocker
if "!blprotected!"=="1" (
    echo.
    echo  !cOrange!BitLocker detectado. Suspendiendo por !bl_rebootcount! reinicio(s)...!cReset!
    manage-bde -protectors -disable %SystemDrive% -rebootcount !bl_rebootcount! >nul 2>&1
    if errorlevel 1 (
        echo  !cRed!No se pudo suspender BitLocker. Abortando.!cReset!
        echo  !cYellow!Usa la opcion [4] Revertir Cambios para restaurar el estado anterior.!cReset!
        echo.
        pause
        exit /b 1
    ) else (
        echo  !cGreen!BitLocker suspendido. El cifrado sigue activo.!cReset!
    )
)
exit /b 0

:: ========================================
:: VERIFICACION FACEIT ANTI-CHEAT
:: ========================================
...
:: ========================================
:: VERIFICACION SMART APP CONTROL
:: Solo Windows 11 build 22621+
:: ========================================
...
if "!hvcirunning!"=="1" set "hvci_status=1"
if "!hvciconfig!"=="1" set "hvci_status=1"
exit /b
...
call :check_vtx
if "!vtx!"=="0" (
    echo  !cRed![ADVERTENCIA] Virtualizacion ^(VT-x/SVM^) NO habilitada en BIOS.!cReset!
...
call :check_dse
if "!dse!"=="0" echo  !cGreen!  Driver Signature Enforcement: DESACTIVADO!cReset!
if "!dse!"=="1" echo  !cYellow!  Driver Signature Enforcement: Test Signing activo!cReset!
if "!dse!"=="2" echo  !cOrange!  Driver Signature Enforcement: ACTIVO ^(necesita F7^)!cReset!
echo.

call :check_uefi_locks
if defined vbslocked       echo  !cOrange!  [UEFI LOCK] VBS bloqueada por firmware ^(SecConfig.efi disponible^)!cReset!
if defined hvcilocked      echo  !cOrange!  [UEFI LOCK] HVCI bloqueada por firmware ^(SecConfig.efi disponible^)!cReset!
if defined cglocked        echo  !cOrange!  [UEFI LOCK] Credential Guard bloqueada por firmware!cReset!
if defined mandatorylocked echo  !cOrange!  [UEFI LOCK] VBS/HVCI en modo obligatorio!cReset!
if defined anylocked echo.
...
call :check_faceit
if defined faceit_detected (
    echo  !cRed!  [ADVERTENCIA] FACEIT Anti-Cheat detectado.!cReset!
    echo  !cYellow!  Puede bloquear la carga del driver. Desinstalalo antes de continuar.!cReset!
    echo.
)
...
if defined sacstate (
    if "!sacstate!"=="0x1" (
        echo  !cGreyHL!  Smart App Control: ACTIVO - puede bloquear aplicaciones.!cReset!
        echo  !cYellow!  Desactivalo en Seguridad de Windows si causa problemas.!cReset!
        echo.
    )
    if "!sacstate!"=="0x2" (
        echo  !cGreyHL!  Smart App Control: En evaluacion - puede activarse automaticamente.!cReset!
        echo  !cYellow!  Se recomienda desactivarlo en Seguridad de Windows.!cReset!
        echo.
    )
)
...
if "!vtx!"=="0" (
    echo  !cRed![ADVERTENCIA] Virtualizacion ^(VT-x/SVM^) NO habilitada en BIOS.!cReset!
    echo  !cYellow!  Activar VBS desde registro no tendra efecto hasta habilitarla en BIOS/UEFI.!cReset!
    echo.
...
call :check_faceit
if defined faceit_detected (
    echo  !cRed![BLOQUEADO] FACEIT Anti-Cheat detectado.!cReset!
    echo  !cYellow!  FACEIT AC bloquea la carga del driver. Desinstalalo antes de continuar.!cReset!
    echo.
    pause
    exit /b
)
...
:: Confirmacion final
choice /C:SN /N /M "  ^!Estas seguro? Esto desactivara TODAS las protecciones. (S/N): "
if errorlevel 2 (
    echo.
    echo  !cYellow!Operacion cancelada!cReset!
    echo.
...
if "!vbs_status!"=="1" (
    ...
    set "rpsfval="
    for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard" /v RequirePlatformSecurityFeatures 2^>nul') do set "rpsfval=%%A"
    if defined rpsfval reg add "HKLM\SOFTWARE\ManageVBS" /v RequirePlatformSecurityFeatures /t REG_SZ /d "!rpsfval!" /f >nul 2>&1
)
...
if "!credguard_status!"=="1" (
    if "!cgrunning!"=="1" reg add "HKLM\SOFTWARE\ManageVBS" /v CredentialGuard /t REG_DWORD /d 1 /f >nul 2>&1
    if "!cgscenario!"=="1" reg add "HKLM\SOFTWARE\ManageVBS" /v CredentialGuardScenario /t REG_DWORD /d 1 /f >nul 2>&1
)
...
if not defined vbslocked (
    ...
    if defined rpsfval reg delete "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard" /v RequirePlatformSecurityFeatures /f >nul 2>&1
    call :disable_vbs
    if not errorlevel 1 set "anythingdisabled=1"
) else (
    echo  !cOrange!  VBS ^(UEFI Lock^): gestionado via SecConfig.efi           [SecConfig]!cReset!
)
...
if not defined hvcilocked (
    ...
    echo  !cOrange!  HVCI ^(UEFI Lock^): gestionado via SecConfig.efi          [SecConfig]!cReset!
)
...
if not defined cglocked (
    ...
    echo  !cOrange!  Credential Guard ^(UEFI Lock^): gestionado via SecConfig.efi [SecConfig]!cReset!
)
...
:: Smart App Control (solo aviso)
call :check_sac
if defined sacstate (
    if "!sacstate!"=="0x1" (
        echo.
        echo  !cGreyHL!  [INFO] Smart App Control ACTIVO.!cReset!
        echo  !cYellow!  Desactivalo en: Seguridad de Windows ^> Control de aplicaciones.!cReset!
    )
    if "!sacstate!"=="0x2" (
        echo.
        echo  !cGreyHL!  [INFO] Smart App Control en evaluacion.!cReset!
        echo  !cYellow!  Se recomienda desactivarlo en Seguridad de Windows.!cReset!
    )
)
...
:: Si nada fue desactivado, informar segun estado DSE
if not defined anythingdisabled (
    if "!dse!"=="0" (
        echo  !cYellow!Todas las protecciones y DSE ya estaban desactivadas. Sin cambios.!cReset!
        echo.
        pause
        exit /b
    ) else if "!dse!"=="1" (
        echo  !cYellow!Todas las protecciones ya desactivadas. Test Signing activo.!cReset!
        echo.
        pause
        exit /b
    ) else (
        echo  !cYellow!Todas las protecciones ya estaban desactivadas. Se procedera al reinicio F7.!cReset!
        echo.
    )
)

echo  !cYellow!NOTA: El sistema reiniciara en el menu de opciones avanzadas.!cReset!
echo  !cCyan!  Selecciona la opcion 7: Deshabilitar firma obligatoria de controladores.!cReset!
echo.
...
:: BitLocker: 2 reinicios si Credential Guard activo en Win10, 1 en el resto
set "bl_count=1"
if "!cgrunning!"=="1" if !winbuild! LEQ 19045 set "bl_count=2"

call :suspend_bitlocker !bl_count!
