...
:: ========================================
:: WINDOWS SECURITY FEATURES MANAGER v3.2
:: ========================================
...
:: NUEVO: Personalizacion individual de caracteristicas
:: CORREGIDO: Reinicio en modo avanzado (F7) sin preguntar, como en V3.1
:: ========================================
...
:: ========================================
:: INFORMACION DETALLADA DEL SISTEMA
:: ========================================
...
:: ========================================
:: DETECCION MULTIPLE DE ANTI-CHEATS
:: ========================================
...
:: ========================================
:: SUSPENDER BITLOCKER (VERSION V3.1, SIN COMPROBACIONES EXTRA)
:: ========================================
...
:: ========================================
:: RESPALDAR CONFIGURACION ACTUAL (MANUAL)
:: ========================================
...
:get_vbs_status
set "vbs_status=0"
for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard" /v EnableVirtualizationBasedSecurity 2^>nul') do if "%%A"=="0x1" set "vbs_status=1"
exit /b

:get_hvci_status
set "hvci_status=0"
for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\HypervisorEnforcedCodeIntegrity" /v Enabled 2^>nul') do if "%%A"=="0x1" set "hvci_status=1"
exit /b

:get_winhello_status
set "winhello_status=0"
for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\WindowsHello" /v Enabled 2^>nul') do if "%%A"=="0x1" set "winhello_status=1"
exit /b

:get_secbio_status
set "secbio_status=0"
for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\SecureBiometrics" /v Enabled 2^>nul') do if "%%A"=="0x1" set "secbio_status=1"
exit /b

:get_sysguard_status
set "sysguard_status=0"
for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\SystemGuard" /v Enabled 2^>nul') do if "%%A"=="0x1" set "sysguard_status=1"
exit /b
...
set "cgrunning=0"
set "cgscenario=0"
for /f "delims=" %%s in ('%psc% "(Get-CimInstance -ClassName Win32_DeviceGuard -Namespace root\Microsoft\Windows\DeviceGuard).SecurityServicesRunning" 2^>nul') do if "%%s"=="1" set "cgrunning=1"
for /f "tokens=3" %%A in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\CredentialGuard" /v Enabled 2^>nul') do if "%%A"=="0x1" set "cgscenario=1"
...
set /p "=  Activando VBS...                        " <nul & call :enable_vbs
set /p "=  Activando HVCI...                       " <nul & call :enable_hvci
set /p "=  Activando Windows Hello...              " <nul & call :enable_winhello
set /p "=  Activando System Guard...               " <nul & call :enable_sysguard
set /p "=  Activando Credential Guard...           " <nul & call :enable_credguard
set /p "=  Activando KVA Shadow...                 " <nul & call :enable_kva
set /p "=  Activando Hypervisor...                 " <nul & call :enable_hypervisor
...
:: ========================================
:: DESACTIVAR TODAS LAS CARACTERISTICAS (CORREGIDA - SIN PREGUNTAS DE REINICIO)
:: ========================================
...
:: --- Suspender BitLocker (sin comprobación de error) ---
call :suspend_bitlocker

:: Configurar reinicio en modo avanzado
bcdedit /set {current} onetimeadvancedoptions on >nul 2>&1
if errorlevel 1 (
    echo.
    echo  !cRed!Error al configurar el reinicio avanzado.!cReset!
    echo  !cYellow!Asegúrate de ejecutar el script como Administrador.!cReset!
    echo.
    pause
    exit /b
)
...
call :get_vbs_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v VBS /t REG_DWORD /d !vbs_status! /f >nul 2>&1
call :get_hvci_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v HVCI /t REG_DWORD /d !hvci_status! /f >nul 2>&1
call :get_winhello_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v WindowsHello /t REG_DWORD /d !winhello_status! /f >nul 2>&1
call :get_secbio_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v SecureBiometrics /t REG_DWORD /d !secbio_status! /f >nul 2>&1
call :get_sysguard_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v SystemGuard /t REG_DWORD /d !sysguard_status! /f >nul 2>&1
call :get_credguard_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v CredentialGuard /t REG_DWORD /d !credguard_status! /f >nul 2>&1
call :get_kva_status && reg add "HKLM\SOFTWARE\ManageVBS\Working" /v KVAShadow /t REG_DWORD /d !kva_status! /f >nul 2>&1
exit /b
